...
def select_dataset(rank, partition, batch_size, args, isTest=False, collate_fn=None):
    ...
    if collate_fn is not None:
        return DataLoader(partition, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=4, drop_last=dropLast, collate_fn=collate_fn)
    return DataLoader(partition, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=4, drop_last=dropLast)
